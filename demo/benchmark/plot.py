...
SED_CMD = R"s/.*_\(.*\)workers.*_\(.*\)solutions_.*took \(.*\)ms to find \(.*\) solution.*best solution \([0-9\.]*\).*/\1,\2,\3,\4,\5/"
CMD = f"""
...
df.columns = ["workers", "required solutions", "time", "solutions", "best cost"]
...
sns.violinplot(x="workers", y="time(s)", data=df, ax=axs[0])
axs[1].set_title("time(s)")

sns.violinplot(y="solutions", x="workers", data=df, ax=axs[1])
...
sns.violinplot(x="workers", y="time/solutions", data=df, ax=axs[2])
